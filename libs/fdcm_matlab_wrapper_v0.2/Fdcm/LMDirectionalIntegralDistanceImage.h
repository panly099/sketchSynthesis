...
inline float LMDirectionalIntegralDistanceImage::Sum(int x1, int y1, int x2, int y2, int& count)
{
	...
	if (xindexed_)
	{
		if (x1 <= x2)
		{
            ...
            if (x1-1<0 || x1-1 > width_ || x2<0 || x2 > width_ || y1+indices_[x2]-indices_[x1]<0 || y1+indices_[x2]-indices_[x1] > height_ || y1-indices_[x1]+indices_[x1-1]<0 || y1-indices_[x1]+indices_[x1-1] > height_) 
                return 1e+10;
		...
		else
		{
            ...
            if (x2-1<0 || x2-1 > width_ || x1<0 || x1 > width_ || y2+indices_[x1]-indices_[x2]<0 || y2+indices_[x1]-indices_[x2] > height_ || y2-indices_[x2]+indices_[x2-1]<0 || y2-indices_[x2]+indices_[x2-1] > height_) 
                return 1e+10;
	...
	else
	{
		if (y1 <= y2)
		{
            ...
            if (x1+indices_[y2]-indices_[y1]<0 || x1+indices_[y2]-indices_[y1] > width_ || x1-indices_[y1]+indices_[y1-1]<0 || x1-indices_[y1]+indices_[y1-1] > width_ || y2<0 || y2 > height_ || y1-1<0 || y1-1 > height_) 
                return 1e+10;
		...
		else
		{
			...
			if (x2+indices_[y1]-indices_[y2]<0 ||x2+indices_[y1]-indices_[y2] > width_ || x2-indices_[y2]+indices_[y2-1]<0 || x2-indices_[y2]+indices_[y2-1] > width_ || y1<0 || y1 > height_ || y2-1<0 || y2-1> height_) 
                return 1e+10;
